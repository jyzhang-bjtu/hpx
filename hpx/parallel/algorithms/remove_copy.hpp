...
namespace hpx { namespace parallel { HPX_INLINE_NAMESPACE(v1)
{
    ...
    namespace detail
    {
        ...
        {
            remove_copy_if(char const* const name = "remove_copy_if")
              : remove_copy_if::algorithm(name)
            {}
    ...
    namespace detail
    {
        /// \cond NOINTERNAL
        template <typename IterPair>
        struct remove_copy : public remove_copy_if<IterPair>
        {
            remove_copy()
                ...
                OutIter dest, const T& val, Proj && proj)
            {
                ...
                return remove_copy_if::sequential(first, last, dest, pred,
                    std::forward<Proj>(proj));
            }
            ...
            {
                with_value<T> pred(val);
                return remove_copy_if::parallel(
